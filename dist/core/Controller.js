 ...
 */var Controller=function(){/**
   ...
   */Controller.getInstance=function getInstance(key){if(null===key)return null;if(null===this.instanceMap[key]){this.instanceMap[key]=new this(key)}return this.instanceMap[key]};/**
   ...
   * @return {void}
   */Controller.prototype.executeCommand=function executeCommand(note){var commandClassRef=this.commandMap[note.getName()];if(commandClassRef===null)return;var commandInstance=new commandClassRef;commandInstance.initializeNotifier(this.multitonKey);commandInstance.execute(note)};/**
   ...
   * @return {void}
   */Controller.prototype.registerCommand=function registerCommand(notificationName,commandClassRef){if(this.commandMap[notificationName]===null){this.view.registerObserver(notificationName,new Observer(this.executeCommand,this))}this.commandMap[notificationName]=commandClassRef};/**
   * Check if a command is registered for a given Notification
   *
   * @param {string} notificationName
   * @return {boolean}
   *  whether a Command is currently registered for the given notificationName.
   */Controller.prototype.hasCommand=function hasCommand(notificationName){return this.commandMap[notificationName]!=null};/**
   ...
   * @return {void}
   */Controller.prototype.removeCommand=function removeCommand(notificationName){if(this.hasCommand(notificationName)){this.view.removeObserver(notificationName,this);this.commandMap[notificationName]=null}};/**
   * @static
